...
// export default function PendingDeliveriesDataTables({
//   columns,
//   data,
//   rowCount,
//   pagination,
...
//   // 2. Updated to match your actual filter key "deliveryStatus"
//   const hasActiveFilters = filters.deliveryStatus;
...
//                       </span>
//                       <button
//                         onClick={() => filters.setDeliveryStatus("")}
...
//                       onCheckedChange={() => filters.setDeliveryStatus(status)}
//                     >
//                       {status.toLowerCase()}
//                     </DropdownMenuCheckboxItem>
//                   ))}
  ...
  return (
    <div className="w-full space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-row items-center justify-between gap-2 bg-slate-50/50 p-2 rounded-xl">
          ...
          <div className="relative flex-1 min-w-0 max-w-[200px] sm:max-w-xs md:max-w-sm">
            ...
            <Input
              placeholder={searchPlaceholder}
              className="pl-9 pr-10 bg-white border-slate-200 h-9 sm:h-10 shadow-md focus-visible:ring-brand-aqua rounded-lg text-xs sm:text-sm"
              value={globalFilter ?? ""}
              onChange={(e) => setGlobalFilter(e.target.value)}
            />
            ...
            {globalFilter && (
              <button
                ...
                aria-label="Clear search"
              >
                <IconX className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-600 group-hover:text-slate-800 transition-colors" />
              </button>
            )}
          </div>

          {/* RIGHT SIDE: Chips + Divider + Filter Button */}
          <div className="flex items-center justify-end gap-2 shrink-0">
            ...
            {/* Vertical Divider */}
            {hasActiveFilters && (
              <div className="hidden md:block w-0.5 h-6 bg-slate-200 shrink-0" />
            )}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "h-9 sm:h-10 px-2.5 sm:px-3 border-brand-aqua/80 shadow-sm bg-brand-aqua/5 hover:bg-brand-aqua/30 transition-all whitespace-nowrap",
                    hasActiveFilters &&
                    "border-brand-aqua ring-1 ring-brand-aqua"
                  )}
                >
                  ...
                  />
                  <span className="text-xs sm:text-sm font-medium text-slate-700 ml-1.5">
                    Filters
                  </span>
                  {hasActiveFilters && (
                    <span className="ml-1.5 flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-brand-aqua text-[9px] sm:text-[10px] text-white font-bold">
                      {Number(!!filters.deliveryStatus)}
                    </span>
                  )}
                </Button>
        ...
        {/* MOBILE + TABLET CHIPS: Shown below main bar on smaller screens */}
        <div className="flex md:hidden flex-wrap gap-2 px-1">
          <AnimatePresence>
            {hasActiveFilters && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
            ...
            <TableHeader className="bg-slate-50/50">
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow key={headerGroup.id}>
                    <TableHead
                      key={header.id}
                      className="text-slate-700 px-4 sm:px-6 font-semibold h-10 bg-slate-100 text-[11px] sm:text-xs whitespace-nowrap"
                    >
                      {flexRender(
                        header.column.columnDef.header,
                        header.getContext()
      ...
      {/* --- PAGINATION SECTION --- */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-2 border-t border-slate-100 mt-3 sm:mt-4 px-1 md:px-0">
        {/* Result count */}
        ...
        <div className="flex items-center justify-center sm:justify-end gap-3 sm:gap-4 md:gap-6">
          {/* Rows per page */}
          <div className="flex items-center gap-2">
            <Label className="hidden sm:block text-[10px] font-bold text-slate-400 uppercase whitespace-nowrap">
              Rows
            </Label>
            <Select
              value={`${table.getState().pagination.pageSize}`}
              onValueChange={(value) => table.setPageSize(Number(value))}
            >
              <SelectTrigger className="h-8 w-[62px] sm:w-[70px] text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {[10, 20, 50].map((size) => (
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="text-xs font-semibold px-2 whitespace-nowrap">
              {table.getState().pagination.pageIndex + 1} /{" "}
              {table.getPageCount()}
            </div>
            <Button
