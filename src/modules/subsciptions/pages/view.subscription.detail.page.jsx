...
export default function ViewSubscriptionDetailPage() {
  ...
  const { userDetail, userDetailLoading, actionLoading } = useSelector(
  ...
  const handleManualGrant = async () => {
    const finalReason =
      grantReasonType === "Other" ? grantReasonCustom : grantReasonType;
              ...
              <CardContent className="p-6 space-y-8">
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    ...
                    <Badge
                      variant="secondary"
                      className="bg-brand-aqua/10 text-brand-aqua text-[10px] font-black py-1 px-2 uppercase tracking-widest"
                    >
                  ...
                  <Progress
                    value={progress}
                    className="h-2 rounded-full bg-brand-aqua/10 [&>div]:bg-brand-aqua shadow-inner"
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-3.5 pt-1">
                    <InfoRow
                      ...
                      isGreen={sub?.autoRenew}
                    />
                    <InfoRow
                      label="Platform"
                      val={sub?.platform?.toUpperCase() || "ADMIN_DIRECT"}
                    />
                    <InfoRow label="Source" val={getSource(sub)} />
                    <InfoRow
                      label="Product ID"
                      val={sub?.productId || "manual_grant_v1"}
                      isMono
                    />
                  </div>
                    ...
                    <>
                      ...
                      <div className="space-y-3">
                        <p className="text-[10px] font-black text-brand-aqua uppercase tracking-widest flex items-center gap-1.5">
                          <CalendarPlus className="w-3.5 h-3.5" /> Admin
                          Extensions ({extensionLogs.length})
                        </p>
                        ...
                        auditLogs.map((txn, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell>
                                <span className="text-[11px] font-bold text-slate-500 truncate max-w-[120px] block">
                                  {txn.reason || "—"}
                                </span>
                              </TableCell>
                          ...
                          >
                            <TableCell className="pl-8 py-3">
                              <Badge
                                className={cn(
                                  "text-[10px] font-black px-2 py-0 h-5 shadow-none border",
                                  hist.planType?.includes("1_MONTH") ||
                                    hist.planType === "MONTHLY"
                                    ? "bg-blue-50 text-blue-600 border-blue-100"
                                    : hist.planType?.includes("3_MONTH") ||
                                        hist.planType === "QUARTERLY"
                                      ? "bg-indigo-50 text-indigo-600 border-indigo-100"
                                      : hist.planType === "MILESTONE"
        ...
        <DialogContent className="rounded-[2.5rem] p-0 max-w-sm border-none shadow-2xl font-jakarta overflow-hidden">
          <div className="bg-gradient-to-br from-brand-aqua to-brand-aqua/80 p-6 text-center">
            ...
            <DialogHeader>
              ...
              <DialogDescription className="text-white/70 text-xs font-bold uppercase tracking-widest text-center">
                Current Balance: {wallet?.superKeensBalance || 0}
              </DialogDescription>
            </DialogHeader>
          </div>
          <div className="p-6 space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <Input
                  ...
                  value={superKeenAmount}
                  onChange={(e) => setSuperKeenAmount(Number(e.target.value))}
                  min={1}
                />
                <Button
                  variant="outline"
            ...
            <div className="p-3 rounded-xl bg-brand-aqua/5 border border-brand-aqua/20">
              ...
              <p className="text-sm font-black text-brand-aqua">
                {(wallet?.superKeensBalance || 0) +
                  (Number(superKeenAmount) || 0)}{" "}
                Super Keens
              </p>
            </div>
          </div>
          <div className="px-6 pb-6">
            <Button
              className="w-full bg-brand-aqua text-white font-black h-12 rounded-2xl hover:bg-brand-aqua/90 transition-all text-xs uppercase tracking-widest shadow-xl shadow-brand-aqua/20"
              onClick={() =>
                handleGrantConsumables("SUPER_KEEN", superKeenAmount).then(() =>
                  setIsSuperKeenOpen(false),
                )
              }
        ...
        <DialogContent className="rounded-[2.5rem] p-0 max-w-sm border-none shadow-2xl font-jakarta overflow-hidden">
          <div className="bg-gradient-to-br from-brand-aqua/90 to-brand-aqua/60 p-6 text-center">
            ...
            <DialogHeader>
              ...
              <DialogDescription className="text-white/70 text-xs font-bold uppercase tracking-widest text-center">
                Current Balance: {wallet?.boostsBalance || 0}
              </DialogDescription>
            </DialogHeader>
          </div>
          <div className="p-6 space-y-5">
            ...
            <div className="p-3 rounded-xl bg-brand-aqua/5 border border-brand-aqua/20">
              ...
              <p className="text-sm font-black text-brand-aqua">
                {(wallet?.boostsBalance || 0) + (Number(boostAmount) || 0)}{" "}
                Profile Boosts
              </p>
            </div>
          </div>
          <div className="px-6 pb-6">
            <Button
              className="w-full bg-brand-aqua text-white font-black h-12 rounded-2xl hover:bg-brand-aqua/90 transition-all text-xs uppercase tracking-widest shadow-xl shadow-brand-aqua/20"
              onClick={() =>
                handleGrantConsumables("BOOST", boostAmount).then(() =>
        ...
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 max-w-md font-jakarta">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs font-bold uppercase tracking-widest text-slate-400">
              Assign premium access to user profile
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5 py-6">
            <div className="space-y-2">
              ...
              <Select value={grantPlan} onValueChange={setGrantPlan}>
                ...
                <SelectContent className="rounded-xl border-none shadow-xl">
                  <SelectItem value="MONTHLY" className="text-sm font-bold">
                    Monthly Plan
                  </SelectItem>
                  <SelectItem value="YEARLY" className="text-sm font-bold">
                    Yearly Plan
                  </SelectItem>
                  <SelectItem value="LIFETIME" className="text-sm font-bold">
                    Lifetime Tier
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={setGrantReasonType}
              >
                <SelectTrigger className="h-12 rounded-xl bg-slate-50 border-none font-bold text-sm">
                ...
                <SelectContent className="rounded-xl border-none shadow-xl">
                  <SelectItem
                    value="1000th User Reward"
                    className="text-sm font-bold"
                  >
                    1000th User Reward
                  </SelectItem>
                  <SelectItem value="Giveaway" className="text-sm font-bold">
                    Giveaway
                  </SelectItem>
                  <SelectItem
                    value="Compensation"
                  ...
                  <SelectItem value="Other" className="text-sm font-bold">
                    Other (Custom)
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          <DialogFooter className="gap-3">
            <Button
              variant="ghost"
              className="h-11 rounded-xl font-bold text-xs uppercase tracking-widest px-6"
        ...
        <DialogContent className="rounded-[2.5rem] text-center max-w-sm p-8 border-none shadow-2xl font-jakarta">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-center font-bold text-slate-400 text-sm mt-2">
              This will immediately revoke all subscription privileges. This
          ...
          <DialogFooter className="mt-10 sm:justify-center gap-4">
            <Button
              variant="ghost"
              className="h-12 rounded-2xl font-black text-xs uppercase tracking-widest px-8"
              onClick={() => setIsRevokeOpen(false)}
            >
              Dismiss
            </Button>
            <Button
              className="bg-rose-500 h-12 rounded-2xl px-10 font-black text-white hover:bg-rose-600 text-xs uppercase tracking-widest shadow-2xl shadow-rose-500/30"
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const result = await dispatch(
                  extendSubscription({
                    userId,
                    data: { days: Number(extendDays), reason: extendReason },
                  }),
                );
                if (result.meta.requestStatus === "fulfilled") {
                  toast.success(
      ...
      >
        <DialogContent className="max-w-[95vw] sm:max-w-3xl p-0 overflow-hidden bg-black/95 border-none shadow-2xl rounded-3xl">
          <DialogHeader className="p-4 bg-white/10 backdrop-blur-md border-b border-white/10 flex flex-row items-center justify-between space-y-0 absolute top-0 left-0 right-0 z-50">
            <DialogTitle className="text-white font-black text-xs uppercase tracking-widest">
              {imageModal.title}
            </DialogTitle>
            <Button
              variant="ghost"
