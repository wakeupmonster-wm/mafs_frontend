...
export default function ConfigPage() {
    ...
    const { config, configLoading, actionLoading } = useSelector((state) => state.subscription);
    ...
    const updatePremium = (key, value) => {
        const parsed = parseInt(value, 10);
        setPremiumLimits((prev) => ({ ...prev, [key]: isNaN(parsed) ? 0 : parsed }));
        markChanged();
    };
    ...
    const handleSave = async () => {
        ...
        if (JSON.stringify(freeLimits) !== JSON.stringify({
        ...
        if (JSON.stringify(premiumLimits) !== JSON.stringify({
                    ...
                    >
                        ...
                        <div className="bg-emerald-50/60 border border-emerald-100 rounded-2xl p-4 space-y-3">
                            ...
                            <p className="text-[10px] text-emerald-500 font-medium">
                                Each qualifying user receives {milestone.grantDurationDays} days of free premium access.
                            </p>
                        </div>
                    </ConfigCard>
                </div>
                ...
                <div className="px-2">
                    <div className="bg-slate-800 rounded-[2rem] p-6 flex items-start gap-4 shadow-xl">
                        ...
                        <div className="space-y-1 flex-1">
                            <h4 className="text-sm font-black text-white">How Config Changes Work</h4>
                            <p className="text-xs font-medium text-slate-400 leading-relaxed">
                                Changes take effect <span className="text-brand-aqua font-bold">immediately</span> after saving.
                                Free/Premium limits control daily quotas — existing users will see updated limits on their next session.
                                Feature toggles enable/disable premium features <span className="text-brand-aqua font-bold">globally</span> for all premium users.
                                The milestone program auto-grants premium to first N users — set <span className="text-white font-bold">isActive = false</span> to pause.
                            </p>
                        </div>
                    </div>
                </div>
...
const ConfigCard = ({ icon, emoji, title, subtitle, color, children }) => {
    ...
    return (
        <Card className={cn(
            ...
            <CardHeader className={cn("pb-4 border-b border-slate-50", headerBg[color])}>
                <div className="flex items-center gap-3">
...
const QuotaField = ({ label, description, value, onChange, min, max, icon, allowUnlimited }) => {
    ...
    return (
        <div className="group">
            ...
            {description && (
                <p className="text-[10px] font-medium text-slate-400 mb-2 pl-5">{description}</p>
            )}
            <div className="flex items-center gap-2">
                <Input
                    type="number"
                    className={cn(
                        ...
                        isUnlimited ? "bg-amber-50 text-amber-700" : "bg-slate-50 text-slate-800"
                    )}
                    value={value}
...
const FeatureToggle = ({ label, description, checked, onCheckedChange, icon }) => (
