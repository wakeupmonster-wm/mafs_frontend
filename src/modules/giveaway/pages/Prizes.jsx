...
export default function Prizes() {
  ...
  const { prizes, loading, error, successMessage } = useSelector(
    (s) => s.giveaway
  );
  ...
  const validateForm = () => {
    ...
    if (supportiveItemsArray.length < 2) {
      toast.error(
        "Please provide at least 2 supportive items separated by commas"
      );
      return false;
    }
  ...
  const submit = () => {
    ...
    const supportiveItemsArray = parseSupportiveItems(
      form.supportiveItems.trim()
    );
  ...
  const handleDelete = (prizeId, prizeTitle) => {
    if (
      window.confirm(
        `Are you sure you want to delete "${prizeTitle}"? This action cannot be undone.`
      )
    ) {
  ...
  const filteredPrizes = prizes.filter((prize) => {
    const matchesSearch = prize.title
      .toLowerCase()
        ...
        <Card className="border-0 shadow-lg shadow-gray-200/50 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700 hover:shadow-xl transition-shadow">
          ...
          <CardContent className="p-5 bg-white">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div
                className="space-y-1.5 animate-in fade-in slide-in-from-left-2 duration-500"
              ...
              <div
                className="space-y-1.5 sm:col-span-2 animate-in fade-in slide-in-from-left-2 duration-500"
            ...
            <div
              className="mt-4 flex gap-2 animate-in fade-in slide-in-from-bottom-2 duration-500"
            ...
            >
              ...
              {editingId && (
                <Button
                  onClick={resetForm}
                  variant="outline"
                  className="h-10 gap-2 border-gray-300 hover:bg-gray-50 transition-all hover:scale-105 animate-in fade-in zoom-in-50 duration-300"
                >
                  <X className="h-4 w-4" />
                  Cancel
                </Button>
        ...
        >
          <CardHeader className="border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white pb-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Badge
                variant="secondary"
                className="w-fit bg-amber-100 text-amber-700 border border-amber-200 animate-in fade-in zoom-in-50 duration-500"
              >
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <div
                className="relative sm:col-span-2 animate-in fade-in slide-in-from-left-2 duration-500"
                style={{ animationDelay: "100ms" }}
              >
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {filteredPrizes.map((p, index) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition-all duration-200 hover:scale-105 ${getTypeStyles(
                                p.type
                              )}`}
                            >
                              {getTypeIcon(p.type)}
