...
//   "giveaway/fetchAllWinners",
//   async (_, { rejectWithValue }) => {
//     try {
//       const res = await getAllWinnersApi();
...
//     try {
//       const res = await getAllWinnersApi();
//       // res = { success: true, count: 34, data: Array(34) }
//       // Array res.data mein hai ✅
//       return res.data;
//     } catch (err) {
//       return rejectWithValue(
//         err.response?.data?.message || "Failed to fetch winners"
...
export const fetchAllWinners = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      const res = await getAllWinnersApi(params);
      return res;  // { success, data, pagination, stats }
    } catch (err) {
      return rejectWithValue(
        err.response?.data?.message || "Failed to fetch winners"
      );
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllWinners.fulfilled, (state, action) => {
        ...
        state.winnersPagination = action.payload.pagination || {
          page: 1,
          limit: 10,
          total: 0,
          totalPages: 0,
        ...
        state.winnersStats = action.payload.stats || {
          totalCampaigns: 0,
          completed: 0,
          pending: 0,
          withWinner: 0,
      ...
      //   }
      //   if (winnerData) {
      //     const alreadyExists = state.winners.find(
